...
const handler = async (m, {conn, usedPrefix}) => {
  ...
  if (m.text.toLowerCase().startsWith(usedPrefix + 'acertijo')) {
    ...
    const caption = ` ⷮ *${json.question}* Tiempo restante: ${(timeout / 1000).toFixed(2)} segundos Puntos: +${poin} Exp `.trim();
    conn.tekateki[id] = [ await conn.reply(m.chat, caption, m), json, poin, setTimeout(async () => {
      if (conn.tekateki[id]) await conn.reply(m.chat, `Se acabó el tiempo! La respuesta era: ${json.response}`, conn.tekateki[id][0]);
      delete conn.tekateki[id];
    }, timeout)];
  } else if (m.quoted && m.quoted.fromMe && m.quoted.isBaileys) {
    if (!(id in conn.tekateki)) return m.reply('No estas en juego');
    if ((link unavailable) == conn.tekateki[id][0].id) {
      ...
      if (text === response) {
        ...
        delete conn.tekateki[id];
      } else if (similarity(text, response) >= threshold) {
