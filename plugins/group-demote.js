...
const handler = async (m, { conn, usedPrefix, text }) => {
  if (!m.quoted && !text) return conn.reply(m.chat, `*[ ℹ️ ] Menciona a un usuario para quitar admin.*`, m);
  if (text) {
    var user = text.split`@`[1] + '@s.whatsapp.net';
  } else if (m.quoted.sender) {
    var user = m.quoted.sender;
  } else if (m.mentionedJid) {
  ...
  const admins = await conn.groupAdmins(m.chat);
