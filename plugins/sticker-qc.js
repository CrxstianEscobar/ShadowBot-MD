...
let text
    if (args.length >= 1) {
        ...
        text = m.quoted.text;
    } else throw '*[ ℹ️ ] Agrega un texto.*';
   if (!text) return m.reply(m.chat, '* [ ℹ️ ] Te faltó el texto.*');
    const who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender; 
    const mentionRegex = new RegExp(`@${who.split('@')[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'g');
    const mishi = text.replace(mentionRegex, '');
   if (mishi.length > 30) return m.reply(m.chat, '*[ ℹ️ ] El texto no debe tener mas de 30 caracteres.*');
